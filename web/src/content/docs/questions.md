...
**What is not there yet.** The model says `materialized: table`, but `lakelet question run` does not build that table; it runs the SQL. Building it with `dbt run` hits a limit of DuckDB-Iceberg, not of dbt: the built-in `table` materialisation renames a temp table over the target inside one transaction, which the catalog refuses. [Transactions and the catalog](/docs/transactions) has the full list of what is refused and why, and the materialisation the repo carries for it; how it ships is a pending decision, and until then the checks are tested against a table Lakelet builds.
