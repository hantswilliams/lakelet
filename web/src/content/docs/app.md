...
The welcome screen offers **Open a folder…** and the ten most recent projects. Any folder can be opened: one without a `lakelet.toml` is set up first (`lakelet init <folder>`), and the window shows what was created. A project whose tables live in a bucket ([a warehouse in a bucket](/docs/remote#a-warehouse-in-a-bucket)) starts from **Open… → New folder, tables in a bucket…** in the bar — an `s3://bucket/prefix`, then the folder dialog — or from **Advanced** on the welcome screen: the folder chosen, which must not be a project yet, is set up with `lakelet init <folder> --warehouse s3://…`, with the AWS credentials from the environment the app was started in. A folder that is a project already refuses it, because a warehouse is fixed at init. Each project gets its own window and its own `lakelet serve`, so two projects never share an engine. The first window's core is given 60% of the machine's RAM as its DuckDB memory limit and each further window half of the previous share; the health tiles show the limit this window has. Closing a window stops its core. **Open…** in the bar lists the recent projects and **Other folder…** for the dialog.
