...
| `GET /changes?since=2d&last=50&name=orders` | | `lakelet changes`: the feed, newest first — every table's snapshots, every run of each model and question, the versions git holds for the models — each `{when, kind, name, target, …}` with `kind` one of `snapshot` (`operation`, `snapshot_id`, `added_rows`, `deleted_rows`, `affects`: the models it made out of date), `run` (`ok`, `seconds`, `verdict`, `error`) or `version` (`id`, `author`, `message`, `names`: every model and question the commit touched), and `target` what `name` is: `table`, `model`, `question`, or `project` for a version about several files or none. `since` as the CLI takes it (`2d`, `12h`, `30m`, `1w`, an ISO date); 400 `bad_since`. |
