...
The repo carries a materialisation that only asks the catalog for what it does: when the model's columns are unchanged, `DELETE` then `INSERT` in one transaction, keeping the table; when they changed, `DROP` then `CREATE` as separate transactions. It is exercised by `core/tests/test_step6_materialisation.py` on three consecutive `dbt run`s. How it ships, as an override of `table` written into the project's `macros/` by `init` or as an explicit `lakelet_table`, is a pending decision in `build-sessions/decisions-for-review_090926.md`; until it lands, [saved questions](/docs/questions) are run by Lakelet and not built by `dbt run`.
