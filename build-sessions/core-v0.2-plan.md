...
> **Superseded by `core-v0.3-plan.md` on September 8, 2026.** Do not build from this file.

*Plan revision 4 · September 8, 2026 · This file is `core-v0.2-plan.md`. It supersedes `core-v0.1-plan.md` (revision 3), which carried the pending R1 to R6 block on remote data and the four-minute demo. All six were accepted on September 8 in `decisions-for-review_090826.md` and are applied here as D25 to D27 and the changes they imply. Decisions in §1 are made; the rest of the file follows from them. §9 lists what changed and the evidence behind each change.*
...
Each one states the call, the reason, and what it costs. Decisions marked *(amended)* changed from the original brief. D19 to D24 were added in revision 3 after the review. D25 to D27 are the September 8 decisions on remote data. Numbers are referenced from the rest of the file.
...
| Remote, read-only | Attach a Parquet prefix, or an existing Iceberg metadata location, in `s3://` as a read-only Iceberg table with the user's own AWS credentials (D25); metadata local by default, in the bucket by flag (D26); `refresh` picks up new files (D27); bandwidth probe; Red verdicts with the bandwidth sentence |
...
lakelet tables attach <name> <s3-prefix or metadata-location> [--metadata-in-bucket] | refresh <name>
...
| `docs/lakelet-day0-prd.md` | F0.7.2: "embedded" means a loopback HTTP server in the process (D4). F0.3.6: history is `.lakelet/history.db` with the §3.4 schema; SQL text is stored locally and never shared (D7). F0.4.5: `last_run` lives in history, not in the `.sql` file (D16). F0.1.4: the preview shows the Iceberg type each column becomes (D24). F0.6.1: `run`, `publish` and `catalog attach` are not in the first CLI (D11). F0.6.4: v0 exit codes are 0, 1, 2, 4; 3 arrives with burst (D23). F0.7.4: `catalog attach` is Day 1 (D11). F0.1: add the story "point at a bucket prefix, get an Iceberg table without copying" (D25). D2 demo step 3: the bucket is Lakelet-owned and prepared with `tables attach` (D27). Add: `init` writes `.gitignore` (D14). C2: `/api` bearer token, `/v1` open on loopback in local mode (D22). D3: the `file://` fallback, if ever needed, is a minimal S3 subset inside the sidecar over local disk, not a bundled MinIO, which is no longer maintained |
