...
- [ ] `web/src/data/nav.ts` still points the GitHub link at `https://github.com`; point it at the repo.
- [ ] Domain: when bought, add under Settings → Pages and set `SITE_URL`.
- [ ] Developer docs on the site: not started. The CLI verbs, `lakelet.toml`, the catalog's REST surface and the API are all built and tested; see the 2026-09-09 log for the recommendation on when and how.
