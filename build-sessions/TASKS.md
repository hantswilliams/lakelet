...
5. **Session 9, the rest** (`versions-plan.md`, G1 to G11 decided 2026-09-12): git commits on save and on run through dulwich, `lakelet versions` and `restore`, save-as-question from the query screen, screen 9 as the model panel's Versions section, table-level lineage (D32). **In progress: steps 0 to 2 built and verified on the Mac; steps 3 to 5 to go — see the table in "Now".** Hants' call 2026-09-12: this comes before ship, while he uses the app in parallel.
...
| 9 | dbt and the Simple/Technical screens, widened with real S3 and three app screens | **in progress**; the real-data round's steps 0 to 6 built 2026-09-11; versions, save from the app and lineage are `versions-plan.md`, G1 to G11 decided 2026-09-12, steps 0 to 2 built and verified on the Mac, steps 3 to 5 to go | `real-data-plan.md`, `versions-plan.md` |
