...
*Amended September 15, 2026 by `trust-round-plan.md` T6: "JSON POSTs without CORS are not reachable from a browser page" was true only of `application/json`; the server now refuses, before any route, a non-loopback `Host` (DNS rebinding), any `Origin` on `/v1` or one CORS does not allow on `/api`, and any body that is not JSON. The token on `/v1` stays with the team catalog. `PRIVACY.md` exists from that day.*
...
*Amended September 15, 2026 by `trust-round-plan.md` T2: a registered file rewritten under the same path is detected (size against the manifest, modification time against the last verification) by `refresh` and `describe`, named, and `refresh` refuses until `tables attach --replace` registers the prefix again.*
...
**D32. Lineage in Day 0 means table level.** Which models and tables feed which, from the dbt manifest and Iceberg snapshot history. Built in session 9, exposed as `lakelet lineage <table>`, `GET /api/lineage/{name}` and the MCP `lineage` tool, which returns "no dbt project" until session 9 exists. Column-level lineage stays Day 3 (product spec F3.5). *Built September 16, 2026 (`versions-plan.md` step 4, `lakelet/lineage.py`): from the manifest and the catalog, with how each edge is known; the MCP tool will call `lineage.lineage(project, name)`.*
...
- *Amended September 9, 2026, from the materialisation spike (`tests/test_step6_materialisation.py`).* Against the catalog: `CREATE OR REPLACE TABLE` is refused by DuckDB-Iceberg; drop-then-create inside one transaction is refused; `DELETE` then `INSERT` inside one transaction works and keeps the table (two snapshots); the dbt swap works when each statement commits on its own. A Lakelet materialisation that rebuilds in place when the columns are unchanged and drops and recreates in separate transactions otherwise builds a model through the catalog on three consecutive `dbt run`s. A root project can override the built-in `table` materialisation in dbt 1.12 without a warning. Decided the same day (`decisions-for-review_090926.md`, all three accepted) and shipped as a step 6 amendment: `init` writes `macros/lakelet.sql`, an override of dbt's `table` for the project; `dbt run` builds saved questions through the catalog and `dbt test` passes their checks; `test_step6_materialisation.py` is the gate. §3.2's layout gains `macros/lakelet.sql`. Either strategy leaves old data files behind; snapshot expiry is Day 1.
