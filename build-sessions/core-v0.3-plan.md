...
> **Superseded by `core-v0.4-plan.md` on September 8, 2026.** Kept as the record of the M1 to M12 review; do not build from this file.

*Plan revision 5 · September 8, 2026 · This file is `core-v0.3-plan.md`. It supersedes `core-v0.2-plan.md` (revision 4), which applied the six remote-data decisions as D25 to D27. Revision 5 aligns the core with what the deck and the site already promise, after a line-by-line comparison on September 8. Twelve preliminary decisions from that comparison are applied provisionally in the body and listed for review in the block below. Decisions in §1 are made; the rest of the file follows from them. §9 lists what changed and the evidence; §10 lists what changes on the site and in the deck.*

---

> **Open for review.** Twelve preliminary decisions from comparing this brief with `deck/` and `site/` on September 8. Each is already applied in the body, provisionally, and says where it landed. Tick *Agree* to keep it. Tick *Change* and write a line, and revision 6 reverts or adjusts that spot. The comparison itself is in `lakelet-build-sessions_090826.md` §5.

## M. Pending decisions: aligning the core with the deck and the site

### M1. The verdict words are the site's words

**Recommend:** the gauge line prints "Runs here", "Runs here, slowly" and "Needs more machine". Green, Yellow and Red stay as the enum in history, JSON output, thresholds and the code.
**Why:** every page, every mockup and every deck slide uses the site's words. The brief printed the colour names. PRD C3 only asks for a word next to the dot.
**Landed in:** D28, §3.6 gauge lines, step 5.

- [x] Agree
- [ ] Change:

### M2. The burst half of the sentence is computed in gauge v0, by arithmetic

**Recommend:** worker size from the ladder on the pricing page, burst wall time from bytes and a cold-start allowance, cost from list price plus 15 percent, cap from PRD F0.5.3. Labelled as estimates. Session 8 replaces the assumptions with measured ones.
**Why:** every Yellow and Red line on the site ends with "burst ~40 s · cap $0.08". Without it the demo sentence is half a sentence until session 8, and two calibration columns stay empty. It is a pure function of numbers the gauge already has; no network, no control plane.
**Landed in:** D29, D8, step 5, §3.6.

- [x] Agree
- [ ] Change:

### M3. Core surface the app and the agents already assume, added now

**Recommend:** `tables discover s3://bucket/` (candidate prefixes with sizes, one listing call); `describe` returns columns, types, partitioning, freshness and last commit; a `leased_until` column in the catalog's table row; `estimate -f`; `--format parquet`; `/api/health` returns versions, machine profile and cached bandwidth; `lakelet audit network`.
**Why:** each one is on a mockup or in the MCP tool table today. Each is an hour or two now and a schema migration or a session-6 detour later.
**Landed in:** D3, D17, §3.6, steps 1, 3, 7, 8, 9.

- [x] Agree
- [ ] Change:

### M4. A saved question is a dbt model from step 6

**Recommend:** `question save` writes `models/questions/<slug>.sql` (plain SQL is a valid model) plus a `schema.yml` entry with the title as description and two default checks. `init` writes a minimal `dbt_project.yml` when none exists. Git commits naming the author stay in session 9.
**Why:** the deck and app screens 7 to 9 say every project is a dbt project under git from the first save. The brief followed PRD F0.4.5 and wrote `questions/*.sql` with a YAML header. Two formats for one thing is a migration in session 9. Writing the dbt shape from day one costs nothing.
**Landed in:** D30, D14, D16, §3.2, §3.5, step 6, §8.

- [x] Agree
- [ ] Change:

### M5. Iceberg format version: accept 2 and 3, write 2 until a writer asks for 3

**Recommend:** the catalog accepts both. Tables import creates are V2. Step 1 records what DuckDB requests on create and what `MERGE INTO` writes. The site's "Iceberg V3" chip becomes "Apache Iceberg" until import produces V3.
**Why:** no document had chosen, and D24's nanosecond truncation is V2 behaviour. Claiming V3 on the site before a V3 table exists is the kind of claim a partner checks.
**Landed in:** D31, step 1, §7, §10.

- [x] Agree
- [ ] Change:

### M6. A half-day dbt spike in step 2, and MCP moves after burst

**Recommend:** as soon as attach and `USE` work, point dbt-duckdb at the attached catalog and run one incremental merge model. Record the result in §7 either way. In the session plan, session 5 (MCP) runs after session 8 (burst).
**Why:** dbt is the most advertised feature (the medallion page, the landing page's `lakelet run --burst auto`) and the last one built, with an open unknown. The four-minute demo needs sessions 6, 7, 8 and 9 and does not use MCP; nothing depends on MCP's position.
**Landed in:** step 2, D15, §5, §7, §8.

- [x] Agree
- [ ] Change:

### M7. Simple correction factors in session 10, not Day 1

**Recommend:** a per-machine, per-operator-class ratio applied after twenty recorded runs, in session 10. Trained models on shared data stay Day 1.
**Why:** PRD F0.3.6 makes it a MUST, screen 5 shows "learned on this machine", and the pricing page says every run makes the next estimate better. The table already exists; the reader is a small function once runs exist, and partners produce runs from week 8.
**Landed in:** D8, §2 Out, §8.

- [x] Agree
- [ ] Change:

### M8. SQL an agent ran is stored locally; the site's `history` flag means sharing

**Recommend:** D7 stands. The agents page and screen 10 currently say "SQL an agent ran is not kept unless history is on". Redefine that flag as sharing or sync, and say that SQL text stays in `history.db` on the machine and never leaves it.
**Why:** the same pages promise "everything it did" and "see exactly what it ran". You cannot show what it ran without storing it. Storing locally and never sharing is the position that satisfies both sentences.
**Landed in:** no core change; §10.

- [x] Agree
- [ ] Change:

### M9. Lineage in Day 0 means table level

**Recommend:** which models and tables feed which, from the dbt manifest and Iceberg snapshot history, built in session 9 and exposed as a verb, a route and the MCP `lineage` tool. Column-level lineage stays Day 3.
**Why:** lineage is in the free tier's feature list, the Day 0 MCP tool list and deck slide 7, and in no build plan; the product spec puts it in Day 3. The manifest's DAG makes the table-level version small.
**Landed in:** D32, §2 Out, §8.

- [x] Agree
- [ ] Change:

### M10. `init` installs the DuckDB extensions and says so; nothing fetches at query time

**Recommend:** `lakelet init` installs `iceberg`, `httpfs` and `excel`, prints one line naming the download, and caches them. `lakelet audit network` runs the local quickstart with outbound connections blocked and reports any attempt. Session 10's installers bundle the extensions so `init` has nothing to fetch. The site's "zero network calls" sentence gains "after install".
**Why:** deck slide 8 and the topologies page promise zero network calls until a bucket or key is added, and the topologies page promises the audit command. A first-query download breaks the promise silently. A first-init download that announces itself keeps it honest until the installers bundle.
**Landed in:** D33, D15, step 2, step 7, §6, §10.

- [x] Agree
- [ ] Change:

### M11. `lakelet ask` is a CLI verb, shipped in session 7

**Recommend:** keep the hero terminal as it is and add the verb to session 7. It is the same ask-box core with a terminal in front.
**Why:** the first thing a visitor sees on the landing page is `$ lakelet ask "…"`. Neither the PRD nor the brief has the verb.
**Landed in:** §2 Out, §8.

- [x] Agree
- [ ] Change:

### M12. The site and deck wording changes in §10 are applied as written

**Recommend:** the fourteen edits in §10: `catalog attach` marked Day 1; the GitHub link handled until Day 1; spot marked "after the beta" and the receipt repriced on on-demand; bring-your-own-account marked later and deck slides 8 and 9 reconciled; GCS and R2 marked later; Snowflake and Athena via the Team catalog; the local data cache removed; "a failed test blocks the commit" softened; Tableau and Metabase corrected; the free-tier Schedule button removed; "DuckDB 1.5"; "Apache Iceberg"; one reference bandwidth; the `history` flag per M8.
**Why:** a partner reads the site before the onboarding call. Each of these is a sentence a partner can check in the beta and find untrue.
**Landed in:** §10.
...
| The site and deck were compared line by line with this brief (September 8) | Twelve gaps between what is advertised for the beta and what any document builds; the comparison is in `lakelet-build-sessions_090826.md` §5 | D28 to D33, M1 to M12, §10 |
...
The site is what a partner reads before the onboarding call. Each line below is a sentence a partner could check in the beta. M12 applies them as written.
...
| `index`, `lakelet-app`; PRD C1 | Reference machine at 190 Mbps on the site, 200 Mbps in the PRD | Pick one |
...
These edits are not applied; the site and deck are the founder's, and M12 is the decision to apply them.
