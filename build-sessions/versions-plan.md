...
*Built September 12 with one correction of fact: the `returns_rows` generic test is not in `macros/lakelet.sql` (that file is `init`'s `table` materialisation) but in `tests/generic/returns_rows.sql`, written by the first save of a project, and that is the third path a first save commits. Nothing else in G3 changed. The log of September 12, §1, has the detail.*
...
**G11. `lakelet run` tells dbt not to send usage statistics, and `audit network` runs it.**
*Added September 12, after step 0, from a failure on Hants' Mac. `dbt-core` 1.12 has `DEFAULT_SEND_ANONYMOUS_USAGE_STATS = True` and ships a Snowplow tracker that posts over HTTPS; Lakelet did nothing about it, so `lakelet run` — the one verb that hands the work to someone else's code — phoned dbt Labs on every invocation. `lakelet audit network` did not catch it, because its quickstart is init, import, estimate, sql, question save and question run, and never invokes dbt: the zero was true of everything it covered and silent about `lakelet run`. That is the deck's central claim failing quietly, so: `dbt/runner.py`'s `_invoke` sets `DO_NOT_TRACK=1`, which is dbt's own switch and sets `SEND_ANONYMOUS_USAGE_STATS` to false; the profile Lakelet writes for a `dbt` run by hand carries `config: send_anonymous_usage_stats: false`, so the path `/docs/dbt` documents is covered too; the audit's quickstart gains a `lakelet run`, reported on its own line, and says so when dbt is not installed rather than failing. A user who wants to send dbt statistics runs `dbt` themselves. Tested on the effect, not the environment variable: after a plan, dbt's flag is false and its tracker is inert. Decided by Hants 2026-09-12.*
- [x] Agree
- [ ] Change:
