...
// button with the CLI line it stands for. A folder is one of these per file.

import type { ImportMode, Preview } from '../lib/api';
  ...
  return (
    <div className="preview-file" data-testid={`preview-${preview.name}`}>
      <table className="columns">
        <thead><tr><th>Column</th><th>DuckDB</th><th>Iceberg</th><th>Note</th></tr></thead>
        <tbody>
...
export function PreviewPanel({ path, folder, previews, name, mode, exists, busy, error, onName, onImport, onCancel }: PreviewPanelProps) {
  const line = importCommand(path, mode, folder ? undefined : name);
  return (
